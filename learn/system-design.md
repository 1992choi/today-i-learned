  ...
  - CA (Consistency + Availability)
    ...
    - 예시 
      ...
      - PostgreSQL (단일 인스턴스 기준)
        - Streaming Replication으로 확장하는 경우 분산 시스템이 되며, 동기화 방식에 따라 성향이 달라짐
        - 기본값인 비동기 복제(Async Replication)는 Standby의 반영 지연을 허용하므로 AP 성향에 가까움
        - synchronous_standby_names 설정으로 동기 복제(Synchronous Replication)를 사용하면 Standby의 커밋 확인을 기다려야 하므로, 분할 시 쓰기가 지연/실패할 수 있어 CP 성향에 가까워짐
...
- 캐시란?
  ...
  - 원본 데이터 저장소(DB 등)에 대한 접근을 줄여 성능을 개선하는 목적
  - 메모리 기반 저장소를 주로 사용하여 낮은 지연 시간 제공
- 캐시를 어떻게 구성할 것인가
  - 앱 서버 캐시 (= 각각의 앱 서버 내에 구성, Local Cache)
    - 각 애플리케이션 서버 내부 메모리에 캐시를 저장하는 방식
    - 네트워크 비용이 없어 가장 빠름
    - 서버 간 캐시 동기화가 어렵고, 인스턴스가 늘어나면 캐시가 분산됨
  - 독립적인 캐시 레이어 구성 (= 별도의 캐시 서버 구성)
    - 애플리케이션과 분리된 전용 캐시 서버를 두는 방식
    - 여러 서버가 동일한 캐시를 공유 가능
    - 네트워크 비용이 발생하지만 일관된 캐시 관리 가능
- 주요용어
  - 캐시 히트 / 미스
    - Cache Hit: 요청한 데이터가 캐시에 존재하여 바로 응답하는 경우
    - Cache Miss: 캐시에 데이터가 없어 원본 저장소에서 조회 후 캐시에 저장하는 경우
  - TTL(Time To Live)
    - 캐시 데이터의 유효 기간을 의미
    - TTL이 지나면 데이터는 자동으로 만료되어 삭제되거나 갱신됨
  - LRU(Least Recently Used)
    - 가장 오랫동안 사용되지 않은 데이터를 우선적으로 제거하는 캐시 교체 정책
    - 메모리 제한이 있는 환경에서 효율적으로 공간 관리 가능

### 지역성
- 지역성이란?
  - 프로그램이 특정 데이터나 메모리 주소를 일정 패턴에 따라 집중적으로 접근하는 특성
  - 캐시 메모리와 같은 계층적 메모리 구조가 성능을 향상시킬 수 있는 근거가 되는 개념
  - 최근에 접근한 데이터 또는 인접한 데이터에 다시 접근할 확률이 높다는 전제에 기반
- 지역성의 종류
  - 시간 지역성
    - 최근에 접근한 데이터에 다시 접근할 가능성이 높은 특성
      - 예: 반복문에서 동일한 변수에 계속 접근하는 경우
    - 캐시는 최근 사용 데이터를 유지함으로써 성능을 개선함
  - 공간 지역성
    - 특정 데이터에 접근하면, 그 주변 주소의 데이터도 접근할 가능성이 높은 특성
      - 예: 배열을 순차적으로 탐색하는 경우
    - 캐시는 인접한 메모리 블록을 함께 로드하여 성능을 향상시킴

### 캐시 패턴
- 쓰기 전략
  - Write Through
    - 데이터를 캐시에 먼저 쓰고, 동시에 원본 저장소(DB)에도 함께 기록하는 방식
    - 항상 캐시와 DB의 데이터가 일관되게 유지됨
    - 쓰기 지연이 발생할 수 있음
    - 데이터 정합성이 중요한 시스템에 적합
  - Write Back
    - 데이터를 먼저 캐시에만 기록하고, 일정 시점에 모아서 DB에 반영하는 방식
    - 쓰기 성능이 빠르고 DB 부하 감소
    - 캐시 장애 시 아직 DB에 반영되지 않은 데이터는 유실될 위험 존재
    - 쓰기 빈도가 높고 성능이 중요한 경우에 적합
  - Write Around
    - 데이터를 DB에 직접 기록하고, 쓰기 시점에는 캐시에 저장하지 않는 방식
    - 이후 해당 데이터가 조회될 때 Cache Miss가 발생하면, 그 시점에 DB에서 읽어와 캐시에 저장함 (Lazy Loading)
    - 불필요한 캐시 오염(Cache Pollution) 방지 가능
    - 한 번 쓰고 자주 조회되지 않는 데이터에 적합

### CDN (Content Delivery Network)
- CDN 이란?
  - 전 세계 여러 지역에 분산된 서버(Edge Server)에 콘텐츠를 복제해두고, 사용자와 가장 가까운 서버에서 콘텐츠를 제공하는 네트워크
  - 주로 이미지, 동영상, CSS, JS와 같은 정적 리소스를 캐싱하여 전달
  - 사용자 요청은 지리적으로 가까운 엣지 서버에서 처리되어 지연 시간을 줄임
- 필요성
  - 속도
    - 사용자와 물리적으로 가까운 서버에서 응답하여 네트워크 지연(Latency) 감소
    - 대용량 정적 파일 전송 시 체감 성능 향상
  - 서버 부하 분산
    - 원본 서버(Origin Server) 대신 CDN이 요청을 처리하여 트래픽 감소
    - 피크 트래픽 상황에서도 안정적인 서비스 제공 가능
  - 가용성, 안정성
    - 특정 지역 서버 장애 시 다른 엣지 서버로 우회 가능
    - DDoS 공격 완화 기능 제공
  - 비용 절감
    - 원본 서버의 트래픽 및 대역폭 사용량 감소
    - 인프라 확장 비용 절감
- 캐시 무효화 (CDN은 콘텐츠를 캐싱하기 때문에, 원본 데이터가 변경되었을 때 기존 캐시를 갱신해줄 필요가 있음)
  - 수동 Purge
    - 관리자가 특정 파일 또는 경로를 강제로 삭제하여 즉시 갱신
    - 즉각 반영 가능하지만 운영 비용 발생
  - TTL 기반 만료
    - 설정된 Time To Live 시간이 지나면 자동으로 캐시 갱신
    - 관리가 단순하지만 즉시 반영은 어려움
  - 버전 관리
    - 파일명이나 쿼리스트링에 버전을 포함하여 새로운 리소스로 인식하게 함
    - 예: app.js?v=2
    - 캐시 무효화 없이도 안전하게 배포 가능

### 캐시 설계시 고려사항
- Hotspot 문제
  - Hotspot 문제란?
    - 특정 키(데이터)에 요청이 과도하게 집중되어 캐시 또는 DB에 부하가 몰리는 현상
    - 인기 상품, 메인 페이지, 실시간 랭킹 등 단일 키에 트래픽이 집중될 때 발생
    - 단일 노드 장애 또는 병목으로 이어질 수 있음
  - 해결방안
    - 데이터 복제와 분산
      - 동일 데이터를 여러 캐시 노드에 복제하여 부하를 분산
    - 로컬 캐시 병행
      - 중앙 캐시 외에 애플리케이션 서버 로컬 캐시를 함께 사용하여 요청 분산
    - 적절한 TTL
      - 너무 짧은 TTL은 만료 시점에 트래픽 폭증(Cache Stampede)을 유발할 수 있으므로 적절한 만료 시간 설정
    - 요청 병합
      - 동일 키에 대한 동시 요청을 하나로 묶어 DB 조회를 1회만 수행하도록 처리
- Celebrity 문제
  - Celebrity 문제란?
    - 팔로워 수가 매우 많은 사용자(연예인, 인플루언서 등)처럼 특정 엔티티에 트래픽이 집중되는 현상
    - 게시물 업로드나 피드 조회 시 대량의 읽기/쓰기 요청이 동시에 발생
  - 해결방안
    - 샤딩
      - 사용자 또는 데이터 기준으로 분산 저장하여 특정 노드에 부하가 몰리지 않도록 설계
    - 로컬 캐시 병행
      - 자주 조회되는 사용자 정보는 애플리케이션 레벨 캐시에 저장
    - 비동기 업데이트
      - 피드 반영을 동기 처리하지 않고 메시지 큐 기반으로 비동기 처리
    - fanout 최적화
      - Fanout-on-Write
        - 게시물이 작성되는 시점에 모든 팔로워의 피드에 미리 데이터를 생성해두는 방식
        - 읽기 성능은 빠르지만, 팔로워가 많을수록 쓰기 비용이 급증
      - Fanout-on-Read
        - 게시물은 작성자 기준으로만 저장하고, 사용자가 피드를 조회할 때 팔로우 목록을 기반으로 데이터를 조합하는 방식
        - 쓰기 비용은 낮지만, 읽기 시 계산 비용 증가
- Warmup 문제
  - Warmup 문제란?
    - 캐시 서버 재시작, 신규 인스턴스 배포 등으로 인해 캐시가 비어 있는 상태에서 트래픽이 한꺼번에 DB로 몰리는 현상
    - 초기 Cache Miss가 대량 발생하여 원본 DB에 급격한 부하 발생
  - 해결방안
    - 데이터 선제로딩
      - 서비스 시작 전에 주요 인기 데이터들을 미리 캐시에 적재
    - 점진적 트래픽 증가
      - 카나리 배포(Canary Deployment) 등으로 신규 인스턴스에 트래픽을 단계적으로 늘려 급격한 부하를 방지
    - 캐시 복제
      - 기존 캐시 노드의 데이터를 신규 노드에 복제하여 초기 미스율을 낮춤

### 메시지 큐
- 메시지 큐 란?
  - 시스템 간 데이터를 비동기적으로 전달하기 위한 중간 저장소
  - 생산자(Producer)가 메시지를 큐에 넣으면, 소비자(Consumer)가 이를 가져가 처리하는 구조
  - 서비스 간 결합도를 낮추고(Decoupling), 트래픽 완충(Buffer) 역할 수행
  - 비동기 처리, 트래픽 분산, 장애 격리에 활용
- 기본 컴포넌트
  - Producer
    - 메시지를 생성하여 큐(또는 토픽)에 전송하는 주체
    - 메시지 생성 후 소비 결과를 기다리지 않고 다음 작업 수행 가능
  - Consumer
    - 큐에 저장된 메시지를 가져와 처리하는 주체
    - 처리 완료 후 ACK를 보내거나 오프셋을 커밋하여 처리 완료를 명시
  - Broker
    - 메시지를 저장하고 전달하는 중간 서버
    - 메시지 저장, 라우팅, 전달 보장 등의 역할 수행
- 메시지 전달 모델
  - Point to Point
    - 하나의 메시지를 하나의 Consumer가 처리하는 모델
    - 작업 분산(Work Queue)에 적합
    - Consumer 중 하나만 메시지를 가져감
  - Publish Subscribe
    - 하나의 메시지를 여러 Consumer가 각각 수신하는 모델
    - 이벤트 브로드캐스트, 알림 시스템 등에 적합
    - 토픽 기반으로 메시지 전달
- 메시지 보존 방식
  - 메모리
    - RAM에 메시지를 저장
    - 속도는 빠르지만 장애 발생 시 유실 가능성 존재
    - 짧은 수명의 데이터에 적합
  - 디스크
    - 파일 시스템에 메시지를 저장
    - 장애 발생 시에도 데이터 복구 가능
    - 메모리 대비 지연은 존재하지만 안정성 높음

### 메시지 큐 처리 방식
- 모델
  - Push 모델
    - 브로커가 Consumer에게 메시지를 능동적으로 전달하는 방식
    - Consumer는 별도 요청 없이 메시지를 수신
    - 실시간성이 높지만, Consumer 처리 속도를 초과하면 부하 발생 가능
  - Pull 모델
    - Consumer가 브로커에 주기적으로 요청하여 메시지를 가져오는 방식
    - Consumer가 처리 속도를 조절할 수 있음
    - 구현이 단순하고 부하 제어에 유리
- 큐 타입
  - Standard Queue
    - 높은 처리량을 목표로 설계된 일반 큐
    - 메시지 순서가 항상 보장되지는 않음
    - 중복 메시지가 발생할 가능성 존재 (At-least-once)
  - FIFO Queue
    - First In First Out 순서를 보장하는 큐
    - 메시지 순서와 중복 제거를 보장
    - 처리량은 Standard보다 낮을 수 있음
- DLQ (Dead Letter Queue)
  - 개념
    - 정상적으로 처리되지 못한 메시지를 별도로 보관하는 전용 큐
    - 일정 횟수 이상 처리 실패, 타임아웃 초과 등의 메시지를 분리하여 저장
    - 문제 메시지로 인해 전체 큐 처리가 지연되는 것을 방지하는 목적
  - 설계시 고려사항
    - 모니터링
      - DLQ에 메시지가 쌓이는 것은 처리 실패가 발생하고 있다는 신호이므로 지속적인 모니터링 필요
      - 알람 설정을 통해 일정 건수 이상 적재 시 즉시 대응 가능하도록 설계
    - 분리
      - 실패 메시지를 메인 큐와 분리하여 시스템 전체 흐름을 보호하는 것이 핵심 목적
      - 보존 기간을 적절히 설정하여 오래된 메시지가 무한정 쌓이지 않도록 관리 필요
      - 즉, “분리”는 단순히 큐를 나눈다는 의미가 아니라, 장애 격리(Isolation)와 책임 분리를 통해 시스템 안정성을 높이기 위한 설계 전략임

### 메시지 큐 사용사례
- 이메일 발송 시스템
  - 회원가입, 비밀번호 재설정, 마케팅 메일 발송 등을 비동기 처리
  - 사용자의 요청과 메일 발송 로직을 분리하여 응답 속도 개선
  - 대량 발송 시에도 트래픽을 큐에 적재하여 점진적으로 처리 가능
- 주문 처리 및 재고 관리
  - 주문 생성 → 결제 → 재고 차감 → 배송 요청 등의 과정을 비동기 이벤트로 처리
  - 각 단계를 서비스별로 분리하여 확장성과 장애 격리 확보
  - 특정 단계 실패 시 재처리(Retry) 또는 DLQ 활용 가능
- 대용량 데이터 처리 및 ETL
  - 로그 수집, 클릭 데이터, 사용자 행동 데이터 등을 큐에 적재 후 순차 처리
  - ETL(Extract, Transform, Load): 데이터를 추출(Extract) → 변환(Transform) → 저장소에 적재(Load)하는 데이터 처리 과정
  - 실시간 스트리밍 처리 또는 배치 처리에 활용
- MSA간 통신
  - 서비스 간 직접 호출 대신 메시지 기반 비동기 통신 사용
  - 서비스 간 결합도 감소 및 확장성 향상
  - 이벤트 기반 아키텍처(Event-Driven Architecture)에 적합
- 장애격리
  - 특정 서비스 장애가 전체 시스템으로 전파되지 않도록 완충 역할 수행
  - 소비자가 일시적으로 장애가 발생해도 메시지를 큐에 보관하여 복구 후 처리 가능
  - 트래픽 급증 시에도 시스템을 보호하는 버퍼(Buffer) 역할 수행

### 빅데이터
- 빅데이터란?
  - 기존의 전통적인 데이터 처리 시스템으로는 저장·관리·분석이 어려울 정도로 규모가 크고 복잡한 데이터
  - 정형 데이터뿐만 아니라 로그, 이미지, 영상, 소셜 데이터 등 비정형·반정형 데이터를 포함
  - 대용량 데이터를 기반으로 인사이트 도출 및 의사결정에 활용
- 3V
  - Volume (규모)
    - 데이터의 절대적인 양이 매우 큼
    - TB, PB 단위 이상의 대용량 데이터 처리 필요
  - Velocity (속도)
    - 데이터가 생성되고 처리되는 속도가 빠름
    - 실시간 스트리밍 데이터 처리 요구 증가
  - Variety (다양성)
    - 정형, 반정형(JSON, XML), 비정형(텍스트, 이미지, 영상) 등 다양한 형태 존재

### OLTP vs OLAP
- OLTP
  - Online Transaction Processing의 약자
  - 실시간 트랜잭션 처리를 위한 시스템
  - INSERT, UPDATE, DELETE 중심의 짧고 빈번한 쿼리 수행
  - 정규화된 스키마 구조를 사용하여 데이터 무결성과 일관성 보장
  - 예: 주문 처리, 결제, 회원가입 등 운영 시스템
- OLAP
  - Online Analytical Processing의 약자
  - 대규모 데이터 분석 및 집계를 위한 시스템
  - 복잡한 JOIN, GROUP BY, 집계 함수 기반의 읽기 중심 쿼리 수행
  - 비정규화 또는 스타 스키마 구조를 활용하여 분석 성능 최적화
  - 예: 매출 분석, 사용자 행동 분석, 리포트 생성
- 설계시 고려사항
  - OLTP / OLAP의 분리
    - 운영 트랜잭션과 분석 쿼리를 동일 DB에서 처리하면 성능 저하 발생
    - 트랜잭션 DB와 분석 전용 데이터 웨어하우스를 분리하는 것이 일반적
  - 데이터 파이프라인
    - OLTP에서 발생한 데이터를 ETL 또는 스트리밍 방식으로 OLAP 시스템에 전달
    - 배치 처리 또는 실시간 스트리밍 아키텍처 설계 필요
  - CDC
    - Change Data Capture의 약자
    - OLTP DB에서 발생한 변경 사항(INSERT/UPDATE/DELETE)을 로그 기반으로 감지하여 OLAP 시스템으로 전달하는 방식
    - 실시간성이 중요한 경우 특히 중요
    - 전통적인 방식에서는 변경 여부를 알기 위해 주기적으로 DB를 조회(Polling)해야 함
      - 불필요한 조회가 반복되어 비효율적
      - 조회 주기에 따라 데이터 반영 지연 발생 가능
      - 변경 여부를 비교·판단하는 로직이 복잡해 오류 발생 가능성 존재
    - CDC는 DB의 트랜잭션 로그를 기반으로 변경 데이터만 추출하므로
      - 불필요한 전체 조회가 없음
      - 지연을 최소화할 수 있음
      - 데이터 파이프라인을 단순하고 안정적으로 구성 가능

### 분산 처리 시스템 (Distributed System)
- 왜 필요할까?
  - 단일 서버로는 처리하기 어려운 대용량 데이터 처리 필요
  - 트래픽 증가에 따른 확장성 확보
  - 장애 발생 시에도 서비스 지속을 위한 고가용성 확보
  - 데이터 저장과 연산을 여러 노드에 분산하여 성능과 안정성 향상
- Hadoop
  - 대규모 데이터를 분산 저장하고 분산 처리하기 위한 오픈소스 프레임워크
  - 여러 대의 저가형 서버를 묶어 하나의 클러스터처럼 동작
  - 데이터 지역성(Data Locality)을 활용해 저장된 위치에서 연산 수행
  - 핵심 구성 요소
    - HDFS (Hadoop Distributed File System)
      - 대용량 파일을 여러 노드에 블록 단위로 분산 저장하는 파일 시스템
      - 데이터 복제를 통해 장애 허용성(Fault Tolerance) 확보
      - Master-Slave 구조 (NameNode / DataNode)
    - MapReduce
      - 대규모 데이터를 병렬 처리하기 위한 분산 처리 모델
      - Map 단계: 데이터를 분산 처리하여 중간 결과 생성
      - Reduce 단계: 중간 결과를 집계 및 통합
      - 디스크 기반 처리 방식 → 안정적이지만 상대적으로 느림
- Spark
  - Hadoop의 MapReduce보다 빠른 처리를 위해 등장한 인메모리 기반 분산 처리 엔진
  - 배치 처리뿐 아니라 스트리밍, 머신러닝, 그래프 처리까지 지원
  - 반복 연산과 대화형 분석에 강점
  - 핵심 개념
    - RDD (Resilient Distributed Dataset)
      - Spark의 기본 데이터 추상화 개념
      - 분산된 불변(Immutable) 데이터 집합
      - 장애 발생 시 lineage 정보를 통해 재계산 가능
    - Memory 기반 처리
      - 중간 결과를 디스크가 아닌 메모리에 저장
      - 반복 연산에서 MapReduce 대비 훨씬 빠른 성능 제공
      - 필요 시 디스크와 병행 사용 가능

### 배치 처리 vs 스트림 처리
- 배치 처리
  - 개념
    - 일정 시간 동안 데이터를 모아서 한 번에 처리하는 방식
      - 데이터가 계속 쌓이다가 특정 시점(예: 매일 새벽)에 일괄 처리됨
      - 과거 전체 데이터를 기반으로 재계산하는 데 적합
  - 특징
    - 긴 처리 주기
      - 실시간이 아닌 시간/일/주 단위로 실행
      - 최신 데이터 반영까지 지연이 발생
    - 효율적인 대용량 데이터 처리
      - 이미 저장된 대규모 데이터를 한 번에 읽어 병렬 처리
      - 로그 분석, 통계 집계, 정산 작업 등에 적합
    - 높은 처리량
      - 지연 시간보다 전체 처리량(Throughput)에 초점
      - 리소스를 집중적으로 사용해 많은 데이터를 빠르게 소화
    - 복잡한 분석과 집계
      - 대규모 JOIN, GROUP BY, 전체 재계산 등에 강점
      - 데이터 웨어하우스 기반 분석에 적합
  - 장점
    - 효율적인 리소스 처리
      - 트래픽이 낮은 시간대에 작업 수행 가능
      - 클러스터 자원을 계획적으로 사용 가능
    - 안정적
      - 이미 저장된 데이터를 기반으로 처리하므로 예측 가능
      - 실패 시 재실행 구조가 비교적 단순
    - 재처리 용이
      - 전체 데이터를 다시 계산하는 구조가 기본 전제
      - 로직 변경 시 과거 데이터 재계산이 쉬움
  - 단점
    - 실시간성이 떨어짐
      - 즉각적인 이벤트 대응이 어려움
      - 실시간 추천, 이상 탐지에는 부적합
- 스트림 처리
  - 개념
    - 데이터가 발생하는 즉시(또는 매우 짧은 지연으로) 지속적으로 처리하는 방식
      - 이벤트 단위로 처리
      - 데이터가 “끊임없이 흐르는” 구조를 전제로 설계
  - 특징
    - 매우 빠른 처리 속도
      - 이벤트 단위로 즉시 연산 수행
      - 수 ms ~ 수 초 단위 응답 가능
    ...
    - 이벤트 기반
      ...
      - 메시지 큐, 로그 스트림 기반 구조
    - 끊임없이 흐르는 데이터
      - 데이터의 끝이 없음 (Unbounded Data)
      - 윈도우(Window) 개념을 활용해 시간 단위 집계
  - 장점
    - 즉각적인 인사이트
      - 실시간 대시보드, 이상 탐지, 알림 시스템 구현 가능
    - 빠른 대응
      - 사기 탐지, 장애 감지 등 즉시 조치 가능
    - 사용자 경험 향상
      - 실시간 추천, 실시간 피드 반영 등 UX 개선
  - 단점
    - 복잡한 구현
      - 상태 관리(State Management) 필요
      - 순서 보장, 중복 처리 문제 고려
    - 리소스 관리
      - 항상 실행 중인 시스템 → 지속적 자원 사용
    - 정확성 보장
      - Exactly-once 보장 어려움
      - 지연 데이터(Late Event) 처리 필요
- 람다 아키텍처
  - 개념
    - 배치 처리와 스트림 처리를 동시에 사용하는 구조
      - Batch Layer: 전체 데이터를 주기적으로 재계산하여 정확성 확보
      - Speed Layer: 실시간 데이터를 즉시 처리하여 지연 최소화
      - Serving Layer: 두 결과를 통합하여 제공
    - 왜 등장했는가?
      - 배치는 정확하지만 느리고, 스트림은 빠르지만 복잡하고 오류 가능성 존재
      - 두 방식의 장점을 동시에 얻기 위한 절충안
  - 장점
    - 정확성과 빠른 속도 동시 확보
      - 실시간 결과 제공 + 배치 재계산으로 최종 정합성 보장
    - 장애에 강함
      - 배치 레이어를 통해 데이터 복구 가능
    - 데이터 재처리 쉬움
      - 전체 데이터를 다시 계산하여 오류 수정 가능
  - 단점
    - 복잡함
      - 시스템이 2중 구조로 운영됨
    - 중복 코드 & 유지보수 어려움
      - 배치/스트림 로직을 각각 구현해야 함
    - 비용 증가
      - 인프라와 운영 복잡성 증가
- 카파 아키텍처
  - 개념
    - 스트림 처리만으로 시스템을 구성하는 단일 파이프라인 구조
      - 모든 데이터를 로그에 저장
      - 재처리가 필요하면 스트림을 처음부터 다시 재생(replay)
    - 람다의 복잡성을 줄이기 위해 등장
      - 배치 레이어를 제거하여 단순화
  - 장점
    - 단순함 & 빠름
  ...
  - 단점
    ...
    - 매우 복잡한 배치 분석의 어려움
      - 전체 재계산이 비효율적일 수 있음
    - 재처리 시간
      - 로그를 처음부터 다시 읽어야 하므로 시간 소요

### 데이터 거버넌스
- 개념
  - 조직 내 데이터의 생성, 저장, 활용, 폐기까지 전 과정에 대해 정책·표준·절차를 정의하고 관리하는 체계
    - 누가 어떤 데이터를 어떻게 사용할 수 있는지 명확히 정의
    - 데이터의 신뢰성과 일관성을 유지하기 위한 관리 프레임워크
    - 단순 기술 문제가 아닌 조직·정책·프로세스를 포함한 개념
- 필요한 이유
  - 데이터 품질 보장
    - 잘못된 데이터는 잘못된 의사결정을 유발
    - 표준 정의와 검증 프로세스를 통해 정확성·일관성 확보
  - 규제 준수
    - 개인정보 보호법, GDPR 등 법적 규제 대응 필요
    - 감사 추적(Audit Trail)과 접근 기록 관리 필수
  - 비용 절감
    - 중복 데이터 제거
    - 불필요한 저장 및 처리 비용 감소
    - 데이터 탐색 시간 단축
  - 협업 효율
    - 동일한 데이터에 대해 공통 정의 사용
    - 부서 간 데이터 해석 차이 최소화
  - 데이터 보안
    - 민감 정보 보호
    - 접근 권한을 역할 기반(RBAC 등)으로 통제
    - 데이터 유출 및 오남용 방지
- 구성요소
  - 데이터 카탈로그
    - 조직 내 데이터 자산을 목록화한 시스템
    - 어떤 데이터가 어디에 있는지 검색 가능
    - 데이터 설명, 소유자, 사용 목적 등 정보 제공
  - 데이터 계보 (Data Lineage)
    - 데이터가 생성되어 최종 활용되기까지의 흐름 추적
    - 어떤 원천에서 왔고, 어떤 변환을 거쳤는지 파악
    - 오류 발생 시 원인 분석에 유용
  - 데이터 품질 관리
    - 정확성, 완전성, 일관성, 최신성 등을 측정·관리
    - 정기적인 검증 및 품질 지표 관리
  - 메타데이터 관리
    - 데이터에 대한 데이터(정의, 형식, 생성일, 소유자 등) 관리
    - 공통 용어 정의 및 표준화
  - 데이터 보안 및 접근제어
    - 역할 기반 접근 통제(RBAC)
    - 암호화 및 마스킹 처리
...
### 모니터링
- 모니터링의 중요성
  - 시스템의 상태를 지속적으로 관찰하고 이상 징후를 조기에 감지하기 위함
    - 단순히 “서버가 살아있는지” 확인하는 것이 아니라
    - 성능, 오류율, 지연 시간, 리소스 사용량 등을 종합적으로 관찰해야 함
    - 문제를 사후 대응이 아니라 사전 대응으로 전환하는 핵심 수단
  - 조용한 장애
    - 시스템이 완전히 멈추지는 않았지만, 일부 기능이 정상 동작하지 않는 상태
      - 예: 특정 API만 500 에러 증가
      - 메시지 큐에 메시지가 쌓이지만 소비되지 않는 상황
    - 사용자가 먼저 발견하기 전에 지표 기반으로 감지해야 함
      - 에러율, 실패율, 재시도 횟수 등의 모니터링 필요
  - 서서히 죽어가는 시스템
    - 갑작스러운 장애가 아니라 점진적인 성능 저하가 발생하는 경우
      - 메모리 누수로 인한 점진적 응답 지연 증가
      - 스레드/커넥션 풀 고갈
    - CPU, 메모리, GC 시간, 응답 시간 추이를 장기적으로 관찰해야 조기 감지 가능
    - 트렌드 기반 모니터링과 알람 임계치 설정이 중요
  - 트래픽 폭증
    - 예상보다 급격히 요청이 증가하는 상황
      - 이벤트, 마케팅, 외부 이슈 등으로 순간적인 부하 증가
    - QPS(Request per Second), 동시 사용자 수, 네트워크 사용량 모니터링 필수
    - 자동 스케일링(Auto Scaling)과 연계하여 대응
    - 미리 감지하지 못하면 연쇄 장애(Cascading Failure)로 이어질 수 있음

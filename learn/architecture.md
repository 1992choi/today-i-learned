...
- 가장 기본적인 아키텍처 구성 (API 서버 + 캐시 서버 + 데이터베이스)
  ...
  - 데이터베이스
    ...
    - RDBMS 고가용성 및 스케일링 구조
      - Active Stand by 구조
        - 고가용성 (High Availability)을 지원하기 위한 구조로, Active(Master)노드가 다운 되었을때 Stand by 노드가 마스터 노드로 격상되어서 서비스를 정상적으로 제공함
    ...
    - 데이터 베이스 샤딩
      ...
      - 방식
        - Vertical sharding
          - 테이블(기능) 단위로 데이터를 나누어 서로 다른 데이터베이스에 저장하는 방식
        - Horizontal sharding
          - 동일한 스키마를 유지한 채, 특정 키(사용자 ID 등) 기준으로 로우를 나누어 여러 데이터베이스에 분산 저장하는 방식
    - NoSQL
      - Not Only SQL
...
- DDD(Domain Driven Design)
  ...
  - DDD의 필요성
    ...
    - 단점
      ...
      - 애자일 방법론과 충돌
        - DDD는 구조적이고 계획적인 접근 방식, 애자일은 빠른 개발속도와 민첩성을 강조.
- API 게이트웨이
  - 기본구조와 기능
  ...
  - 주요 특징
    ...
    - 라우팅
      - API 요청의 헤더나 본문의 내용에 따라서 백엔드 서비스로 라우팅
    - 오케스트레이션
...
- 분산 트랜잭션
  - 분산 트랜잭션 방식
    ...
    - Saga
      - 각 서비스의 로컬 트랜잭션을 순차적으로 실행하고, 중간에 실패하면 보상 트랜잭션(Compensating Transaction)으로 이전 단계를 되돌리는 방식
      - 마이크로서비스 환경에서는 XA(2PC)가 서비스 간 강한 결합과 잠금으로 인한 성능 저하를 유발하기 때문에, 분산 트랜잭션 처리에 Saga 패턴이 더 널리 사용됨
- 서비스 간 트랜잭션 추적
...
- 에러 처리
  ...
  - Error Stack은 response 메시지에 포함 시키지 않는게 좋다.
    - 어떤 기술 스택을 사용하는지, 파일 위치 등 해킹 가능한 자료가 유출될 수 있기 때문
- 버전 관리
...
### Devops
- Devops란?
  - DevOps는 소프트웨어 개발과 시스템 운영을 결합하여 시스템 개발 주기를 단축하고, 비즈니스 목표에 밀접하게 맞추어 기능, 수정, 업데이트를 자주 제공하는 소프트웨어 개발 프랙티스의 집합이다.
- Devops 팀의 업무
  - 메트릭 정의 및 모니터링
    - 모든것을 수치화하여 모니터링
  ...
  - 용량 관리
    ...
    - 마케팅 캠페인이나, 새로운 기능 배포 등 특정 이벤트로 인하여 급격하게 증가될 수 있는 용량 예측
  ...
  - 변화 관리
    ...
    - 아키텍처 디자인 가이드
      - Devops팀은 개발 설계 단계에서부터 참여하여, 장애 회피를 위한 설계에 대한 가이드를 제공
  - 장애 대응
...
- 시스템 모니터링과 장애 대응
  ...
  - 상세목적
    - 시스템 안정성과 성능 유지
      - 자원 사용량과 응답시간 모니터링을 통하여 과부하나 병목 제거
...
### 테스팅
- 테스트란?
  - 서비스가 요구 사항대로 작동함을 확인하고, 잠재적인 문제를 발견하기 위한 단계
  - 예전에는 개발 완료된 후, 테스트 단계에서 수행
  - 현대에는 CI 단계에서 빌드마다 자동으로 테스트 수행이 추가된다.
  - 테스트의 범위에는 완료된 시스템의 검증 이외에, 법률적 요건과 기타 규제에 대한 검증과정이 포함되기도 한다.
- 정적 테스트와 동적 테스트
  - 정적 테스트
    - 코드를 실행하지않고, 문서와 코드를 검토
    - 코드 리뷰
      - 개발자들끼리 코드 내용을 검토하며, 오류나 개선점을 찾는다.
    - 워크스루(Walkthrough)
      - 개발자가 코드 흐름을 설명하고, 팀원들이 리뷰하면서 문제점을 찾는 방식이다.
    - 정적 분석 도구 사용
      - 자동화된 도구(예: SonarQube, ESLint)로 코드의 잠재적 오류와 결함을 찾는다.
  - 동적 테스트
    - 코드를 실행하면서 정상 작동 여부를 검증
    - 유닛 테스트(Unit Testing)
      - 개별 함수나 모듈의 기능을 검증하는 테스트.
    - 통합 테스트(Integration Testing)
      - 여러 모듈이 상호작용할 때 예상대로 작동하는지 확인하는 테스트.
    - 시스템 테스트(System Testing)
      - 전체 시스템의 기능이 요구사항에 맞게 작동하는지 테스트하는 방식.
      - 성능 테스트.
      - 장애 테스트.
        - 시스템이 장애에 대한 복구성이 있는지 테스트
        - 클라우드 매니지드 서비스 도입으로 마스터 다운 등 인프라 레벨 장애 테스트의 필요성은 줄었지만, 시스템이 커지고 복잡해질수록 애플리케이션/서비스 레벨의 장애 주입 테스트(Fault Injection, Chaos Engineering)는 오히려 더 중요해지는 추세
    - 회귀 테스트(Regression Testing)
      - 코드 수정 이후 기존 기능이 제대로 작동하는지 확인.
- 테스트 프로세스
  1. 테스트 계획
     - 테스트 시나리오 작성 및 계획, 테스트 데이터 작성
  2. 예비 (계통 ) 테스트
     - 테스트가 가능한지 시나리오에 따라 기능이 제대로 작동하는지 검증
  3. 메인 테스트
     - 계획된 테스트 시나리오에 따라 실제 테스트를 수행하고, 결과를 기록하여 결함을 식별
  4. 확인 테스트
     - 문제가 발생하였을 경우, 문제를 해결한후, 제대로 해결 되었는지 확인
  5. 회귀 테스트
     - 다른 기능 테스트시에는 이전에 테스트 했던 기능들을 모두 포함해서 테스트 (사이드 이펙트 확인을 위함)
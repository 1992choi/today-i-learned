...
- 다른 브랜치로 checkout을 해야 하는데, 아직 현재 브랜치에서 작업이 끝나지 않았다면 커밋을 하기가 애매하다.
  이런 경우 stash를 이용하면 작업 중이던 파일을 임시로 저장해두고, 현재 브랜치의 상태를 마지막 커밋 상태로 초기화 할 수 있다.
...
- 브랜치란?
  ...
  - 깃에서 브랜치는 여러 작업을 각각 독립된 커밋 이력으로 진행할 수 있도록 하는 기능이다.
  - 워킹 디렉토리, 인덱스, 로컬/리모트 저장소 자체는 브랜치마다 별도로 존재하는 것이 아니라 하나의 저장소 안에서 공유되며, 브랜치를 체크아웃하면 워킹 디렉토리와 인덱스의 내용이 해당 브랜치의 최신 커밋 상태로 바뀌는 것이다.
...
- Fast-Forward 방식
  ...
  - Merge에 의해서 새로운 커밋이 만들어지지 않고, HEAD의 위치만 이동된다.
- 3-way 방식
  ...
  - 두 브랜치의 공통 조상 커밋과 각 브랜치의 최신 커밋, 총 3개의 커밋을 비교해서 병합하기 때문에 3-way라고 부른다.
...
- 재배치(Rebase)란?
  - base를 재설정한다는 의미이다.
  - 하나의 브랜치가 다른 브랜치에서 파생되서 나온 경우, 다른 브랜치에서 진행된 커밋을 다시 가져와서 base를 재설정하는 것을 의미한다.
- 장점
...
- Merge와의 차이점
  - Merge
    ...
    - 두 브랜치의 이력을 그대로 둔 채 병합 커밋을 새로 만들기 때문에, 커밋 이력이 분기된 모양(비선형)으로 남는다.
  - Rebase
    - ![image](https://github.com/user-attachments/assets/99b622a6-29b8-4082-ac3f-024d513d12f3)
    - 브랜치의 커밋들을 기준 브랜치의 최신 커밋 뒤로 옮겨 붙이기 때문에, 커밋 이력이 하나의 직선(선형)으로 남는다.
...
- git diff [변경전 커밋ID] [변경후 커밋ID]
  - 커밋 간의 파일상태를 비교한다.
  - 커밋 순서를 반대로 입력해도 명령어는 동작하며, 이 경우 변경 방향(추가/삭제)이 반대로 표시된다.
    보통 [변경전 커밋ID]에 이전 커밋을, [변경후 커밋ID]에 이후 커밋을 입력해서 변경된 방향대로 diff를 확인한다.
...
- git revert [취소할 커밋ID]
  ...
  - Ex) git revert ccccccc
    ...
    - 커밋순서가 aaaaaaa -> bbbbbbb -> ccccccc -> ddddddd 이라고 가정할 때, git revert ccccccc를 입력하면 기존 커밋 이력(aaaaaaa ~ ddddddd)은 모두 그대로 유지되고, ccccccc의 변경사항만 취소하는 새로운 커밋이 추가된다.
...
3. 브랜치 생성
   ...
   - (git branch로 브랜치만 생성하면 현재 브랜치가 바뀌지 않으므로, 이후 커밋이 새 브랜치가 아닌 원래 브랜치에 쌓이지 않도록 반드시 checkout까지 해준다.)
4. 수정, add, commit, push
   - 코드 수정
   - git add [파일명]

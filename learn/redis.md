...
- Caching
  ...
  - 로그인 세션(Session)
- Rate Limiter
  - Fixed-Window / Sliding-Window Rate Limiter(요청 제한기)
- Message Broker
  - 메시지 큐(Message Queue)
- 실시간 분석 / 계산
...
- Stream
  ...
  - Kafka와 유사한 기능을 가진다.
  - 엔트리마다 시간순으로 증가하는 고유 ID가 부여되며, 새로운 엔트리는 항상 마지막에 추가되기 때문에 XADD 명령어는 O(1) 시간 복잡도를 갖는다.
...
- BloomFilter
  ...
  - false positive를 반환한다.
    ...
    - 이는 동작원리 때문에 그렇다.
    - BloomFilter는 비트 배열(Bit Array)과 여러 개의 해시 함수를 사용한다.
    - 값을 저장할 때는 하나의 값에 여러 해시 함수를 적용해서 나온 각각의 인덱스 위치의 비트를 1로 설정한다.
    - 값을 조회할 때도 동일한 해시 함수들을 적용해서 나온 인덱스들의 비트가 모두 1인지 확인하는데,
    - 서로 다른 값들이 저장되며 비트들이 겹쳐 채워지다 보면, 실제로 저장한 적 없는 값의 해시 인덱스들도 우연히 모두 1로 채워져 있을 수 있다. 이 경우 포함되지 않은 값을 포함된 것으로 잘못 판단하게 된다.
  - Set과 비교해봤을 때, 실제 값을 저장하지 않기 때문에 매우 적은 메모리를 사용한다.
...
- Redis는 트랜잭션 도중 명령어 실행에 실패해도 Rollback을 지원하지 않는다. 실패한 명령어를 제외한 나머지 명령어는 계속 실행된다.
- 하나의 트랜잭션이 처리되는 동안 다른 클라이언트의 요청이 중간에 끼어들 수 없다.
- 명령어
  - MULTI
    - 트랜잭션을 시작한다.
  - DISCARD
    - EXEC 이전에 큐에 쌓인 명령어를 모두 취소한다. (아직 실행 전이므로 엄밀히는 Rollback과 다르다.)
  - EXEC
    - 변경사항을 적용시킨다.
...
- Online Status
  ...
  - 사용예시
    - 현재 시간에 분을 기준으로 키를 생성한다.
      - SETBIT user:1:30 0 1
        - user 1이 30분에 온라인 상태임을 명시 (key: user:1:30, offset: 0, value: 1)
      - GETBIT user:1:30 0
        - 사용자를 조회하는 쪽에서는 1이 조회되므로 온라인 상태임을 알 수 있다.
    - 만약 1분 후 사용자를 조회하는 쪽에서 GETBIT user:1:31 0으로 명령어가 수행되었을 때는 비트 값이 없으므로 오프라인 상태임을 알 수 있다.
...
- TTL greeting
  - 남은 만료시간을 확인한다.
  - 남은 만료시간이 반환되며, -1은 만료기한이 없음을, -2는 키가 존재하지 않음(만료되어 삭제되었거나 애초에 존재한 적이 없음)을 뜻한다.
...
- MULTI
  - 트랜잭션을 시작한다.
- DISCARD
  - EXEC 이전에 큐에 쌓인 명령어를 모두 취소한다.
- EXEC
  - 변경사항을 적용시킨다.
- 예제
  - MULTI > SET text a > DISCARD > GET text : DISCARD로 취소되어 SET이 실행되지 않았으므로 값을 조회할 수 없다.

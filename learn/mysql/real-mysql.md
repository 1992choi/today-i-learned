...
- 차이점
  ...
  - 최대 저장 길이
    - CHAR(255)
      - 최대 255 문자 저장 가능
    - VARCHAR(16383) (utf8mb4 기준)
      - VARCHAR 컬럼 자체의 한계는 65,535 바이트
      - utf8mb4(문자당 최대 4바이트)에서는 65535 / 4 ≈ 16383 문자가 최대
      - 문자셋에 따라 최대 문자 수는 달라짐 (예: latin1은 문자당 1바이트라 더 많은 문자 저장 가능)
...
- UUID vs B-Tree
  ...
  - UUID + UNIQUE 제약이 성능에 미치는 영향
    ...
    - UUID는 랜덤 값
      - 삽입/조회 위치가 인덱스 전체 범위에 걸쳐 무작위로 분산됨 (특정 범위에 집중되지 않음)
      - B-Tree 탐색 자체는 여전히 O(log n)이지만, 접근 위치가 매번 랜덤하여 버퍼 풀 캐시 적중률이 낮아짐
    - 결과
      - 캐시 미스 증가로 인한 랜덤 I/O 증가
      - 쓰기 성능 저하
- UUID 대체 방법
...
- 인덱싱 되지 않은 컬럼을 OR 조건과 함께 사용
  ...
  - 문제
    - account_type만 인덱스가 걸려있다고 가정할 때, joined_at을 만족하는가를 보기위해서는 풀스캔이 일어날 수 밖에 없음
- 복합 인덱스 선행 컬럼 누락
  - 인덱스

...
### 데드락
- MySQL 데드락 감지
  - DeadLock detection thread가 트랜잭션 간 Lock Graph를 지속적으로 검사
    - 어떤 트랜잭션이 어떤 자원을 점유하고, 어떤 자원을 기다리는지 그래프로 관리
    - 순환 구조(서로가 서로를 기다리는 구조)가 발생하면 데드락으로 판단
  - 단점
    - 트랜잭션 수가 많을수록 그래프 탐색 비용 증가
    - 내부적으로 모든 대기 관계를 추적 → CPU 사용 증가
    - 고트래픽 환경에서는 이 자체가 병목이 될 수 있음
  - 대응
    - 일부 환경에서는 감지 기능 비활성화
      - innodb_deadlock_detect = OFF
    - 대신 timeout 기반으로 처리
      - innodb_lock_wait_timeout = 2~3초 권장
      - 일정 시간 이상 대기 시 실패 처리
      - 단, 즉시 감지가 아닌 "지연된 실패" 방식
- MySQL 데드락 처리
  - Victim 트랜잭션 선택
    - 기준
      - Undo 로그가 적은 트랜잭션
      - 즉, 롤백 비용이 적은 트랜잭션을 선택
  - 처리 방식
    - Victim → 강제 롤백
    - 나머지 트랜잭션 → 계속 진행
  - 특징
    - 배치 vs 서비스 경합
      - 배치는 대량 처리 → Undo 많음 → 롤백 비용 큼
      - 서비스는 단건 처리 → Undo 적음 → Victim 될 가능성 높음
      - 결과적으로 서비스 트랜잭션이 더 자주 실패
- MySQL 데드락 해석 어려움
  - 동일 SQL이라도 Lock이 달라짐
    - 실행 계획에 따라 사용하는 인덱스가 달라짐
    - 인덱스가 다르면 Lock 대상도 달라짐
    - 즉, 같은 쿼리라도 상황에 따라 다른 Lock을 잡음
  - Lock 대상이 단순 레코드가 아님
    - Record Lock
      - 실제 데이터 행
    - Gap Lock
      - 레코드 사이의 "빈 공간"
      - 예: id 10, 20이 있을 때 11~19 구간 잠금
      - 다른 트랜잭션의 INSERT 방지 목적
    - Next-Key Lock
      - Record + Gap을 함께 잠금
    - 즉, 눈에 보이지 않는 영역까지 잠금 대상이 됨
  - 모든 트랜잭션이 기록되지 않음
    - 데드락 발생 시 일부 트랜잭션만 로그에 남음
    - 전체 흐름을 완벽히 재현하기 어려움
  - Lock Lifecycle이 일정하지 않음
    - 기본적으로 트랜잭션 종료 시 해제
    - 하지만 일부 Lock은 중간에 해제되거나 추가로 획득됨
    - 실행 계획 / 조건 / 인덱스 탐색 과정에서 Lock이 동적으로 변함
    - 아주 짧은 타이밍 차이로
      - 어떤 경우는 데드락 발생
      - 어떤 경우는 정상 처리
  - 모든 인덱스가 Lock 대상
    - Clustered Index (PK)
      - 실제 데이터 저장 구조 → 항상 영향
    - Secondary Index
      - 조회 시 사용되면 해당 인덱스에도 Lock 발생
    - Foreign Key
      - 참조 무결성 확인을 위해 추가 Lock 발생
    - 즉, 하나의 쿼리가 여러 인덱스에 Lock을 동시에 걸 수 있음
- 정리
  - 데드락은 단순 쿼리 문제가 아니라
    - 트랜잭션 실행 순서
    - 인덱스 구조
    - Lock 종류 (Record / Gap / Next-Key)
    - 실행 타이밍
    - 이 모든 요소가 결합된 문제
  - 특징
    - 재현 어려움
...
### 커넥션 관리
- Max Connection 설정
  - MySQL에서는 max_connections 라는 설정값을 통해서 Server-side에서 커넥션 수를 설정할 수 있다.
  - 최대 8k ~ 10k 까지 가능하다.
  - 실제 운영 환경에서는 보통 200 ~ 1000 사이로 설정하는 경우가 많다.
    - 트래픽 규모, 서버 스펙(CPU/Memory), 애플리케이션의 Connection Pool 크기에 따라 조정
    - 무작정 크게 설정하면 메모리 사용량 증가 및 컨텍스트 스위칭 비용 증가
- Connection Pool 설정
  - Client-side Pool Max
    - 응용 프로그램의 특성에 따라서 상이하지만
      - Connection pool 사용은 필수이면 좋다.
      - 시작 설정은 Max 20 ~ 30 으로 시작하여, 필요에 따라 늘려가면 된다.
      - Min = Max * (70% ~ 90%)
        - Min = Max 설정이 성능상 좋지만, Connection 부족 현상 판단이 어렵다.
          - 부연 설명
            - Min을 Max와 동일하게 설정하면 항상 커넥션이 충분한 상태를 유지할 수 있어 지연이 줄어드는 장점이 있음
            - 하지만 평소에도 항상 풀을 가득 채워두기 때문에
              - 실제로 커넥션이 부족해서 문제가 발생하는 상황인지 아니면 다른 병목(DB 슬로우, 락 경합 등)인지 구분이 어려워짐
            - 반대로 Min을 Max보다 낮게 두면
              - 트래픽 증가 시 커넥션이 점진적으로 늘어나므로 '커넥션 부족' 상황을 관측 가능
              - 운영 관점에서 문제 원인 파악이 쉬워짐
  - Connection Timeout 설정
    - Connection Open 또는 Pool에서 커넥션을 가져오는 설정
    - 밀리초 이하의 값 설정은 권장하지 않음
    - 인프라 여건에 맞춰서 3~10초 정도의 시간 설정 사용
      - 너무 짧게 가져오는게 문제가 되는 상황
        - 대부분 Connection을 가져오지 못하는 경우, 일반적으로 다른 Fallback 전략이 없고 재시도 전략을 보통 사용
        - 이 경우 Connection 반복 요청으로 오히려 부가적인 자원을 소모하고 서버에 더욱 좋지 않은 상황을 만들 수 있음
        - 만약 다른 Fallback 전략이 있다면, 짧게 가져가도 됨.
  - Query Timeout 설정
    - 취소 및 동일 쿼리 재요청 시 오히려 DB 서버의 부담 가중
    - 쿼리 재요청을 하지 않는다면 Timeout 짧게 할 수 있음
  - Idle Timeout 설정
    - Idle의 의미
      - 일정 시간 동안 아무 쿼리도 실행되지 않고 대기 상태에 있는 커넥션
      - 즉, 생성은 되어 있지만 사용되지 않는 커넥션
    - 에러 회피를 위해 Idle Connection Timeout을 매우 짧게 설정은 권장하지 않음
      - DBMS 서버의 커넥션은 짧게 사용하고 버리는 자원이 아님
      - 최소 20~30분 이상 설정 권장

### 테이블 파티셔닝
- 테이블 파티셔닝이란?
  - 하나의 테이블을 논리적으로는 유지하면서, 물리적으로 여러 파티션(테이블 조각)으로 나누어 저장하는 기법
  - 각 파티션은 독립적인 저장 단위를 가지지만, 사용자 입장에서는 하나의 테이블처럼 조회/수정 가능
- 파티셔닝이 필요한 이유
  - 삭제 가능한 이력 데이터들을 효율적으로 관리
    - 보관 기간이 지난 데이터를 제거할 때 DELETE가 아닌 파티션 DROP으로 처리 가능
      - 명령문 하나로 빠르게 처리
      - 대량 DELETE와 달리 Undo/Redo 부담 없음
      - 디스크 공간 즉시 반환 가능
  - 자원 사용 효율 증가 및 쿼리 성능 향상
    - 날짜 기준으로 파티셔닝하면 각 파티션이 특정 기간 데이터만 보유
    - 특정 기간 조회 시 불필요한 파티션은 스캔하지 않음
      - 이를 '파티션 프루닝'이라고 함
      - 예: 최근 1개월 조회 → 해당 파티션만 접근
      - 결과적으로 디스크 I/O 감소 + 메모리 효율 증가
- 파티셔닝 타입
  - RANGE | RANGE COLUMNS
    - 특정 범위 기준으로 데이터 분할
    - 예: 날짜, 숫자 범위 (id, created_at 등)
    - 가장 일반적으로 사용되는 방식
  - HASH | LINEAR HASH
    - 해시 함수를 이용해 데이터를 균등 분산
    - 데이터 분포를 고르게 하고 싶을 때 사용
    - 특정 범위 조회에는 불리
  - LIST | LIST COLUMNS
    - 특정 값 목록 기준으로 분할
    - 예: region = 'KR', 'US', 'JP'
    - 값 종류가 명확히 구분되는 경우 사용
  - KEY | LINEAR KEY
    - MySQL 내부 해시 함수 기반 분할
    - HASH와 유사하지만 컬럼 값을 기반으로 자동 처리
    - 구현이 간단하지만 제어는 제한적
- 파티션 테이블 사용 제약 및 주의사항
  - 외래키 사용 불가 (InnoDB 기준)
  - 공간 데이터 타입, 전문검색 인덱스 사용 제한
  - 파티션 표현식에 함수 사용 가능하지만 모든 함수가 파티션 프루닝을 지원하지는 않음
  - 테이블의 모든 고유키(Primary Key, Unique Key)에 파티셔닝 기준 컬럼이 반드시 포함되어야 함
    - 이유 : 특정 레코드가 어느 파티션에 속하는지 식별 가능해야 하기 때문
  - WHERE 절에 파티셔닝 기준 컬럼 조건이 없으면 파티션 프루닝 불가 → 전체 파티션 스캔 발생
  - 값이 자주 변경되는 컬럼을 파티션 키로 사용 금지
    - UPDATE 시 파티션 간 데이터 이동 발생
    - 내부적으로 DELETE + INSERT로 처리 → 성능 저하
- 파티션 테이블 사용 예시
  - 파티션 추가
    - 마지막 파티션 이후 범위 추가
      - ALTER TABLE user_log
        ADD PARTITION (PARTITION p202404 VALUES LESS THAN ('2024-05-01'));
    - 기존 파티션 사이에 추가 (REORGANIZE 사용)
      - ALTER TABLE user_log
  ...
  - 파티션 제거
    - ALTER TABLE user_log DROP PARTITION p202404;
  - 파티션 비우기
    - ALTER TABLE user_log TRUNCATE PARTITION p202404;
- 파티션 테이블과 인덱스 사용
  - 일반 보조 인덱스는 자유롭게 생성 가능
  - 모든 파티션에 동일한 구조의 인덱스가 각각 생성됨
    - 즉, 인덱스도 파티션 단위로 분리됨
  - 쿼리 실행 흐름
    - 1. WHERE 조건으로 파티션 프루닝 수행
    - 2. 대상 파티션 결정
    - 3. 각 파티션 내부에서 인덱스 스캔 수행
  - 주의
    - 파티셔닝 키 조건이 없으면

...
- 예시
  ...
  - (1) SELECT id, created_at FROM users; 와   
    ...
    (1)처럼 id와 created_at 컬럼만 조회한다고 하더라도 실제 테이블의 데이터에 접근해야하지만   
...
- 실행 계획 조회
  ...
  - rows : SQL문 수행을 위해 접근하는 데이터의 모든 행의 수 (= 데이터 액세스 수)
    ...
    - `EXPLAIN`에서 보여지는 rows는 실제로 읽은 행 수가 아니라 옵티마이저가 테이블 통계 정보를 바탕으로 추정한 값이다. 실제 값을 확인하려면 `EXPLAIN ANALYZE`를 사용해야 한다.
  - filtered : 필터 조건에 따라 어느 정도의 비율로 데이터를 제거했는 지 의미
    ...
    - rows와 마찬가지로 filtered 역시 옵티마이저의 추정치다.
  - Extra : 부가적인 정보를 제공
...
- 튜닝 후
  - 성능 향상을 위해 인덱스를 생성할 때, department / created_at / 멀티 컬럼 인덱스(순서도 고려)를 고민해봐야한다.
    ...
    - created_at와 department 각 각 생성
      - 30ms 소요
      - type = range
      - possible_keys = idx_department, idx_created_at : 2개의 인덱스를 사용할 수 있다.
...
- 쿼리 실행 결과
  ...
  - SQL문 작성 시 인덱스 컬럼을 가공(함수 적용, 산술 연산, 문자열 조작 등)하면, MySQL은 해당 인덱스를 활용하지 못하는 경우가 많다.

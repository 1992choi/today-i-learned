...
- 관련 리팩토링
  ...
  - 매개변수를 질의 함수로 바꾸기 (Replace Parameter with Query)
    - 매개변수 중 isPremium 이라는 값이 있고, 이 값이 이미 전달받은 다른 매개변수(예: customer)로부터 구할 수 있는 값이라면(예: customer.isPremium()), 굳이 별도의 매개변수로 넘기지 않고 메서드 내부에서 직접 호출해서 구할 수 있다. 이 경우 매개변수를 삭제할 수 있다.
...
- 관련 리팩토링
  ...
  - 질의 함수와 변경 함수 분리 (Separate Query from Modifier)
    - 사이드 이펙트 없이 값을 조회할 수 있는 메서드는 테스트 하기도 쉽고, 메서드를 이동하기도 편하다.
  ...
  - 참조를 값으로 변경 (Change Reference to Value)
    - 값 객체(value object)는 불변(immutable)이며 값으로 비교되기 때문에, 여러 곳에서 공유되더라도 예상치 못한 변경으로 인한 사이드 이펙트가 발생할 확률이 적다. (값 객체가 곧 원시 데이터라는 의미는 아니다. 원시 데이터가 아닌 객체도 불변으로 만들면 값 객체가 될 수 있다.)
    - 어떤 객체의 변경 내역을 다른 곳으로 전파시키고 싶다면 레퍼런스, 아니라면 값 객체(불변 객체)를 사용한다.
...
- 관련 리팩토링
  - 조건부 로직을 다형성으로 바꾸기 (Replace Conditional with Polymorphism)
    - 동일한 조건(주로 타입 코드)으로 분기하는 switch 문이 여러 곳에 반복해서 나타난다면, 그 중 하나를 다형성으로 바꾸고 나머지도 차례로 다형성을 활용하도록 고친다.
    - 이렇게 하면 새로운 타입이 추가되더라도 여기저기 흩어진 switch 문을 찾아 고칠 필요 없이 새로운 서브클래스 하나만 추가하면 된다.
...
- 개요
  ...
  - 게터와 세터가 사용되는 메서드를 찾아보고 '함수 옮기기 (Move Function)'를 사용해서 데이터 클래스로 옮길 수 있다.
- 관련 리팩토링
  - 레코드 캡슐화하기 (Encapsulate Record)
    - 변하는 데이터를 다룰 때는 캡슐화를 진행한다.
    - 자바의 Record를 사용하면, Record는 불변 객체이기 때문에 이러한 리팩토링이 필요없다.
    - public 필드를 사용하는 코드를 private 필드와 게터, 세터를 사용하도록 변경한다.

### 냄새 23. 상속 포기
- 개요
  - 서브클래스가 슈퍼클래스에서 제공하는 메서드나 데이터를 잘 활용하지 않는다는 것은 해당 상속 구조에 문제가 있다는 뜻이다.
- 관련 리팩토링
  - 메서드와 필드 내려주기 (Push Down Method / Field)
    - 기존의 서브클래스 또는 새로운 서브클래스를 만들고 슈퍼클래스에서 메서드와 필드를 내려주면 슈퍼클래스에 공동으로 사용하는 기능만 남길 수 있다.
...
- 개요
  ...
  - 모든 주석이 나쁘다는 것은 아니다. 잘 작성된 주석은 좋은 냄새에 해당하기도 한다.
- 관련 리팩토링
  - '함수 추출하기'를 사용해 설명이 필요한 부분을 별도의 메서드로 빼낸다.
  - '함수 선언 변경하기 (Change Function Declaration)'를 사용해 함수 이름을 재정의할 수 있다.
